...
class PCFP:
    ...
    # alternative init condition -- if left None, initial_values will be used
    init_block: Optional[sp.Expression] = None
    ...
    def copy(self):
        ...
        new_pcfp.init_block = self.init_block
    ...
    def is_loc_possibly_initial(self, loc) -> bool:
        if self.init_block is not None:
            solver = Z3SmtSolver(self.get_manager())
            solver.add(self.init_block)
            for var in loc:
                solver.add(sp.Expression.Eq(var.get_expression(), loc[var]))
            solver.push()
            check_res = solver.check()
            return check_res != SmtCheckResult.Unsat
    ...
    def to_prism_string(self) -> str:
        ...
        if self.init_block is not None:
            res += "\ninit\n\t{}\nendinit".format(str(self.init_block))
        return res
